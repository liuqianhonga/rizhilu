...
**优先参考的专业媒体**：
- 国际：**AP News**、**Al Jazeera**（可直接获取全文）
- 国内：**新华社**、**财新**、**澎湃**

搜索方式（按优先级）：
1. 直接访问 AP News / Al Jazeera 搜索相关关键词，获取一手报道
2. 通过 SearXNG 搜索，但必须回到上述媒体核实
3. 避免使用自媒体、论坛、二手转载

读取 `memory/analyzed-topics.json` 过滤已分析事件。

### 2. 多源交叉核实
- 同一事件至少查 **2个不同来源**，优先使用上述媒体
...
- [ ] **具体数字/结论必须有来源**：文章中提到具体目标价、数据、预测时，必须说明参考依据
  - 价格/点位：必须注明具体数值 + 来源（如"截至X日，WTI原油报$XX/桶，数据来源：Bloomberg"）
  - 概率/预测：必须注明来源（如"据PredictIt预测市场，X%概率..."）
  - 多空思路/配置建议：应引用机构观点（如"高盛/摩根大通维持X观点"）
  - **禁止**：凭空写出具体价格或概率，不注明来源
- [ ] **呈现双方立场**：时事热点涉及双方表态时（如谈判、协议、争议），必须呈现双方立场，不能只写单方面声明
- [ ] **区分声明与事实**：引用一方声明时需加"声称"、"据称"等修饰词，不能写成既定事实
